export const vi = {
  name: {
    groom: 'Sơn Tùng',
    bride: 'Nguyệt Thu',
  },
  overview: {
    countdown: {
      days: 'Ngày',
      hours: 'Giờ',
      minutes: 'Phút',
      seconds: 'Giây',
    },
    groom: {
      label: 'Groom',
    },
    bride: {
      label: 'Bride',
    },
  },
  couple: {
    groom: {
      description:
        'Đối với chúng mình hôn nhân không phải là đích đến cuối của tình yêu mà là khởi đầu của hạnh phúc.',
      label: 'Nhà Trai',
      family: {
        mother: 'Bà. Nguyễn Thị Lý',
      },
    },
    bride: {
      description:
        'Ở thời điểm phù hợp nhất, khoác lên mình bộ váy cưới đẹp nhất, gả cho người đáng tin cậy nhất, là anh.',
      label: 'Nhà Gái',
      family: {
        father: 'Ông. Trần Văn Phát',
        mother: 'Bà. Nguyễn Thị Hồng Vân',
      },
    },
  },
  invitation: {
    title: 'TRÂN TRỌNG KÍNH MỜI QUÝ KHÁCH',
    subTitle: 'ĐẾN DỰ BỮA TIỆC CHUNG VUI CÙNG GIA ĐÌNH CHÚNG TÔI TẠI',
    subRestaurant: 'Sảnh Paris - Tầng 2',
    restaurant: 'Trống Đồng Luxury',
    address: 'Lô 2A12.2, Đường 2/9, Phường Hoà Cường, TP. Đà Nẵng',
    time: 'VÀO LÚC 11 GIỜ 00',
    date: 'NGÀY',
    DD: '20',
    dddd: 'THỨ BẢY',
    MMMM: 'THÁNG 6',
    YYYY: '2026',
    viDate: '(Nhằm ngày 06 tháng 05 năm Bính Ngọ)',
    schedule:
      'Sự hiện diện của quý khách là niềm vinh dự cho gia đình chúng tôi!',
  },
  loveStory: {
    title: 'Chúng mình',
    subTitle: 'Đã đến với nhau như thế nào?',
    theFirstDay: {
      title: 'Lần đầu gặp gỡ',
      description:
        'Tình yêu là sự rung cảm của một tâm hồn khi gặp một tâm hồn đồng điệu, là sự hòa nhịp của hai trái tim.',
    },
    firstLoveDay: {
      title: 'Nhận lời yêu nhau',
      description:
        'Sau những lần hẹn hò em đã nhận lời yêu từ anh, và những ngày sau đó chúng mình cùng nhau trải qua những ngày tháng cùng nhau.',
    },
    theLoveJourney: {
      title: 'Hành trình yêu nhau',
      description:
        'Thời gian yêu nhau của chúng mình không quá dài nhưng cũng không phải ngắn. Đối với em, đó là một hành trình dài với rất nhiều cung bậc cảm xúc.',
    },
    sheSayYes: {
      title: 'Phút giây cầu hôn',
      description:
        'Có những khoảnh khắc chúng ta mãi ghi nhớ như cột mốc trong đời, là khoảng khắc anh ngỏ lời mong em theo anh về nhà.',
    },
    engagementDay: {
      title: 'Ngày Lễ Dạm Ngõ',
      description:
        'Chặng đường nào rồi cũng phải có đích đến. Nếu hạnh phúc là một con đường thì anh và em đang dần đi đến chặng đường cuối trong cuộc hành trình đầy cảm xúc này.',
    },
    theWedding: {
      title: 'Mơ một hạnh phúc',
      description:
        'Cảm ơn và trân trọng tất cả! Mong rằng chúng mình sẽ được sống hạnh phúc từng ngày, từng giờ của hôm nay và về sau. Từ nay chúng ta đã về chung một nhà rồi!',
    },
  },
  event: {
    title: 'Sự kiện',
    subTitle: 'Quan trọng',
    event1: {
      title: 'Lễ Vu Quy',
      location: 'Nhà gái',
      address: 'Hà Huy Tập, P. Thanh Khê, TP. Đà Nẵng',
    },
    event2: {
      title: 'Rước Dâu',
      location: 'Nhà gái',
      address: 'Hà Huy Tập, P. Thanh Khê, TP. Đà Nẵng',
    },
    event3: {
      title: 'Lễ Thành Hôn',
      location: 'Trống Đồng Luxury',
      address: 'Lô 2A12.2, Đường 2/9, P. Hoà Cường, TP. Đà Nẵng',
    },
  },
  intention: {
    label1: 'Thân mời bạn tham gia bữa tiệc,',
    label2: 'đánh dấu ngày chúng mình về chung một nhà!',
  },
  thanks: {
    title: 'Trân trọng',
    content:
      'Cảm ơn mọi người đã luôn đồng hành trong quá trình trưởng thành của Tùng và Thu, cũng đã là những người chứng kiến và ủng hộ câu chuyện tình yêu của đôi bạn trẻ. Hôn lễ này sẽ chỉ trở nên trọn vẹn khi nhận được những lời chúc phúc và đặc biệt là sự có mặt của mọi người trong ngày chung đôi của cô dâu và chú rể.',
  },
  common: {
    backToTop: 'Cuộn đến đầu trang',
    tooltip: 'Hãy bật âm thanh',
  },
}
